...
object ICP3  {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // Spark configuration and create Spark context with Spark configuration
    val sc = new SparkContext(conf)
    // The entry point into Spark functionality is the SparkSession class
    // create SparkSession by SparkSession.builder():
    ...
    // For implicit conversions that is for converting RDDs to DataFrames
    ...
    df.write.format("csv").option("header","true").save("C:\\Users\\Kavya Reddy\\IdeaProjects\\sparkicp3\\output2")


    //Apply Union operation on the dataset and order the output by Country Name alphabetically.
    val df1 = df.limit(5)//dataframe1
    val df2 = df.limit(10)//dataframe2
    println("Dataframe 1")
    df1.show()
    println("Dataframe 2")
    df2.show()
    val unionDf = df1.union(df2)//union operation
    println("Union")
    unionDf.orderBy("Country").show()//order by country
    ...
    val DuplicateDF = spark.sql("select COUNT(*),Country,Timestamp from survey GROUP By Timestamp,Country Having COUNT(*) > 1")
    //println("Check duplicate")
    //val unique=df.dropDuplicates().show()
    val unique= df.groupBy("Timestamp","Age","Gender","Country","state","self_employed","family_history","treatment","work_interfere","no_employees","remote_work","tech_company","benefits","care_options").count.filter($"count">1).show()
    val total=df.count()
    //DupDF.show(50)
    println("Number of duplicate records:"+unique)
    println("Total no of records:"+total)



    //Use Groupby Query based on treatment.

    val treatmentgroup = spark.sql("select count(Country) from survey GROUP BY treatment ")
